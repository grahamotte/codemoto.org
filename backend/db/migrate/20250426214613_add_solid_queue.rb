class AddSolidQueue < ActiveRecord::Migration[8.0]
  ...
  def down
    if foreign_key_exists?(:solid_queue_blocked_executions, :solid_queue_jobs)
      remove_foreign_key "solid_queue_blocked_executions", "solid_queue_jobs"
    end

    if foreign_key_exists?(:solid_queue_claimed_executions, :solid_queue_jobs)
      remove_foreign_key "solid_queue_claimed_executions", "solid_queue_jobs"
    end

    if foreign_key_exists?(:solid_queue_failed_executions, :solid_queue_jobs)
      remove_foreign_key "solid_queue_failed_executions", "solid_queue_jobs"
    end

    if foreign_key_exists?(:solid_queue_ready_executions, :solid_queue_jobs)
      remove_foreign_key "solid_queue_ready_executions", "solid_queue_jobs"
    end

    if foreign_key_exists?(:solid_queue_recurring_executions, :solid_queue_jobs)
      remove_foreign_key "solid_queue_recurring_executions", "solid_queue_jobs"
    end

    if foreign_key_exists?(:solid_queue_scheduled_executions, :solid_queue_jobs)
