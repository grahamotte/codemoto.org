...
class AppsTest < Minitest::Test
  ...
  def test_keeps_existing_keychains_available_while_signing
    ...
    assert_includes commands, Shellwords.join([ "security", "list-keychains", "-d", "user", "-s", keychain, "/keychain/login.keychain-db" ])
